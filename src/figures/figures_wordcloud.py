...
def generate_and_save_wordclouds(df, text_col, output_dir, variant_label):
    ...
    for category in df["Category"].unique():
        ...
        category_name = "ham" if category == 0 else "spam"
        ...
        plt.title(f"Word Cloud for Category: {category} ({variant_label} {category_name})")
        plt.axis("off")

        filename = f"wordcloud_{variant_label}_{category}.png"
